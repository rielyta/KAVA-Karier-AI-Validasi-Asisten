"""
main_hf.py — Versi khusus Hugging Face Spaces (port 7860)
Isi identik dengan main.py, hanya port yang berbeda.
"""
...
try:
    ...
    with open(f"{ARTIFACT_DIR}/scaler.pkl",        "rb") as f: data_scaler   = pickle.load(f)
    with open(f"{ARTIFACT_DIR}/label_encoder.pkl", "rb") as f: label_encoder = pickle.load(f)
    with open(f"{ARTIFACT_DIR}/skill_gap_db.json", "r")  as f: skill_gap_db  = json.load(f)
...
class CVPayload(BaseModel):
    ...
    has_education:    int   = Field(..., ge=0, le=1)
...
def _prepare_features(data: CVPayload):
    ...
    num_feat = np.array([[
        min(data.experience_years, 40), skills_count_est,
        has_cert, data.cert_count, data.has_education,
        is_fresh, is_exp, 1
    ]], dtype=np.float32)
...
def _get_top3_predictions(text: str, num_scaled: np.ndarray) -> list:
    ...
    probs      = kava_model.predict(
        {'text_input': text_token, 'numeric_input': num_scaled}, verbose=0
    )[0]
    top3_idx = np.argsort(probs)[-3:][::-1]
    return [
        {"rank": int(r), "role": label_encoder.inverse_transform([int(i)])[0],
         "confidence": round(float(probs[i]) * 100, 2)}
        for r, i in enumerate(top3_idx, 1)
    ]
...
def _analyze_skill_gap(cv_skills_raw: str, predicted_role: str) -> dict:
    ...
    return {
        "required_skills": required, "matched_skills": matched,
        "missing_skills": missing[:10], "coverage_pct": coverage,
...
Profil Kandidat:
- Role diprediksi: {top3[0]['role']} ({top3[0]['confidence']}% confidence)
- Pengalaman: {data.experience_years} tahun
- Skills: {data.skills_raw}
- Skills perlu ditingkatkan: {', '.join(gap['missing_skills'])}

Berikan dalam Bahasa Indonesia (maks 200 kata):
1. **Saran Karir**: Langkah konkret berikutnya
2. **Skill Prioritas**: 3 missing skill terpenting dan alasannya
3. **Rekomendasi Sertifikasi**: 1 sertifikasi internasional paling relevan"""

        resp      = requests.post(
            "https://api.mistral.ai/v1/chat/completions",
            headers={"Authorization": f"Bearer {MISTRAL_API_KEY}", "Content-Type": "application/json"},
            json={"model": MISTRAL_MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": 400},
            timeout=30
        )
        resp.raise_for_status()
        coach_text = resp.json()["choices"][0]["message"]["content"]

        return {"status": "success", "top3_role_matches": top3,
                "skill_gap": gap, "ai_career_coach": coach_text}
...
def skill_gap_only(role: str, skills_raw: str):
    try:
        return {"status": "success", "skill_gap": _analyze_skill_gap(skills_raw, role)}
    except Exception as err:
