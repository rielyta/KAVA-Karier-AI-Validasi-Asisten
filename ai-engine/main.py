...
# FASTAPI APP + CORS (wajib agar Frontend bisa akses API)=
app = FastAPI(
...
class FocalLoss(tf.keras.losses.Loss):
    def __init__(self, gamma=2.0, smoothing=0.05, name='focal_loss', **kwargs):
...
class CVPayload(BaseModel):
    text: str               # Teks gabungan dari CV
    skills_raw: str         # Skills mentah dipisah koma (untuk skill gap)
    experience_years: float
    cert_count: int
    has_education: int      # 1 = punya info pendidikan, 0 = tidak
